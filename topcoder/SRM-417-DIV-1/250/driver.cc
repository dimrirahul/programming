...
string	suffix;
		...
		__in >> __expected >> text >> prefix >> suffix;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << text << ", " << prefix << ", " << suffix << std::endl;
		std::cout.flush();

		__timer_start();

		TemplateMatching __object;
		string __actual = __object.bestMatch(text, prefix, suffix);
