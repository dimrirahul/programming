...
#include "SRMRoomAssignmentPhase.cc"
...
int	K;
		...
		__in >> __expected >> ratings >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << ratings << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		SRMRoomAssignmentPhase __object;
		int __actual = __object.countCompetitors(ratings, K);
