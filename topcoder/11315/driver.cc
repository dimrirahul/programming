...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> sticker;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << sticker << std::endl;
		std::cout.flush();

		__timer_start();

		CubeStickers __object;
		string __actual = __object.isPossible(sticker);
