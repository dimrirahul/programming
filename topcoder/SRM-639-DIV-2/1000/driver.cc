...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> paper;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << paper << std::endl;
		std::cout.flush();

		__timer_start();

		BoardFoldingDiv2 __object;
		int __actual = __object.howMany(paper);
