...
int	t2;
		...
		__in >> __expected >> st1 >> p1 >> t1 >> st2 >> p2 >> t2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << st1 << ", " << p1 << ", " << t1 << ", " << st2 << ", " << p2 << ", " << t2 << std::endl;
		std::cout.flush();

		__timer_start();

		ElectronicPetEasy __object;
		string __actual = __object.isDifficult(st1, p1, t1, st2, p2, t2);
