...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> population;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << population << std::endl;
		std::cout.flush();

		__timer_start();

		SlimeXSlimesCity __object;
		int __actual = __object.merge(population);
