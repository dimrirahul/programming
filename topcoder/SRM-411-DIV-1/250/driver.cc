...
#include "SentenceDecomposition.cc"
...
vector <string>	validWords;
		...
		__in >> __expected >> sentence >> validWords;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << sentence << ", " << validWords << std::endl;
		std::cout.flush();

		__timer_start();

		SentenceDecomposition __object;
		int __actual = __object.decompose(sentence, validWords);
