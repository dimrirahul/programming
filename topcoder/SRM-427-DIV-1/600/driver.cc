...
int	multi;
		...
		__in >> __expected >> K >> multi;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << K << ", " << multi << std::endl;
		std::cout.flush();

		__timer_start();

		LocateTreasure __object;
		string __actual = __object.location(K, multi);
