...
#include "SentenceCapitalizerInator.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> paragraph;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << paragraph << std::endl;
		std::cout.flush();

		__timer_start();

		SentenceCapitalizerInator __object;
		string __actual = __object.fixCaps(paragraph);
