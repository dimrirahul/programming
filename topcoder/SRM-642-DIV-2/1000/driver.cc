...
long long	B;
		...
		__in >> __expected >> N >> X >> Y >> height >> B;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << X << ", " << Y << ", " << height << ", " << B << std::endl;
		std::cout.flush();

		__timer_start();

		TallShoes __object;
		int __actual = __object.maxHeight(N, X, Y, height, B);
