...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> price;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << price << std::endl;
		std::cout.flush();

		__timer_start();

		ImageTraders __object;
		int __actual = __object.maxOwners(price);
