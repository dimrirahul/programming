...
int	maxSize;
		...
		__in >> __expected >> groups >> minSize >> maxSize;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << groups << ", " << minSize << ", " << maxSize << std::endl;
		std::cout.flush();

		__timer_start();

		CreateGroups __object;
		int __actual = __object.minChanges(groups, minSize, maxSize);
