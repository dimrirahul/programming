...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> start;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << start << std::endl;
		std::cout.flush();

		__timer_start();

		PairingPawns __object;
		int __actual = __object.savedPawnCount(start);
