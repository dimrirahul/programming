...
#include "LonglongestPathTree.cc"
...
vector <int>	L;
		...
		__in >> __expected >> A >> B >> L;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << A << ", " << B << ", " << L << std::endl;
		std::cout.flush();

		__timer_start();

		LonglongestPathTree __object;
		long long __actual = __object.getLength(A, B, L);
