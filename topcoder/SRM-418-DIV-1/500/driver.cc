...
vector <int>	price;
		...
		__in >> __expected >> n >> demand >> price;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << demand << ", " << price << std::endl;
		std::cout.flush();

		__timer_start();

		StampsCollection __object;
		int __actual = __object.sell(n, demand, price);
