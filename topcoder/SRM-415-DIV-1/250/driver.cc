...
vector <string>	boxes;
		...
		__in >> __expected >> cranes >> boxes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << cranes << ", " << boxes << std::endl;
		std::cout.flush();

		__timer_start();

		ShipLoading __object;
		int __actual = __object.minimumTime(cranes, boxes);
