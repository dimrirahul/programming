...
#include "CollectingPostmarks.cc"
...
int	K;
		...
		__in >> __expected >> prices >> have >> values >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << prices << ", " << have << ", " << values << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		CollectingPostmarks __object;
		int __actual = __object.amountOfMoney(prices, have, values, K);
