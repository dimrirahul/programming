...
int	rival;
		...
		__in >> __expected >> N >> you >> rival;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << you << ", " << rival << std::endl;
		std::cout.flush();

		__timer_start();

		KnockoutTourney __object;
		int __actual = __object.meetRival(N, you, rival);
