...
int	K;
		...
		__in >> __expected >> shuffle >> maxShuffles >> cardsReceived >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << shuffle << ", " << maxShuffles << ", " << cardsReceived << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		ShufflingMachine __object;
		double __actual = __object.stackDeck(shuffle, maxShuffles, cardsReceived, K);
