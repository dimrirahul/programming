...
int	high;
		...
		__in >> __expected >> low >> high;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << low << ", " << high << std::endl;
		std::cout.flush();

		__timer_start();

		DistinctDigits __object;
		int __actual = __object.count(low, high);
