...
#include "TheProgrammingContestDivTwo.cc"
...
vector <int>	requiredTime;
		...
		__in >> __expected >> T >> requiredTime;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << T << ", " << requiredTime << std::endl;
		std::cout.flush();

		__timer_start();

		TheProgrammingContestDivTwo __object;
		vector <int> __actual = __object.find(T, requiredTime);
