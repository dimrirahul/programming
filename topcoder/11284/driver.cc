...
int	paramB;
		...
		__in >> __expected >> nA >> nB >> paramA >> paramB;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << nA << ", " << nB << ", " << paramA << ", " << paramB << std::endl;
		std::cout.flush();

		__timer_start();

		FoxPlayingGame __object;
		double __actual = __object.theMax(nA, nB, paramA, paramB);
