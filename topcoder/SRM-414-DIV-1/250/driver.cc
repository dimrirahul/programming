...
int	openTime;
		...
		__in >> __expected >> forms >> dayLength >> openTime;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << forms << ", " << dayLength << ", " << openTime << std::endl;
		std::cout.flush();

		__timer_start();

		Embassy __object;
		int __actual = __object.visaApplication(forms, dayLength, openTime);
