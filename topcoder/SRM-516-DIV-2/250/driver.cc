...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> message;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << message << std::endl;
		std::cout.flush();

		__timer_start();

		NetworkXZeroOne __object;
		string __actual = __object.reconstruct(message);
