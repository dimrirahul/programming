...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> current;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << current << std::endl;
		std::cout.flush();

		__timer_start();

		SpecialStrings __object;
		string __actual = __object.findNext(current);
