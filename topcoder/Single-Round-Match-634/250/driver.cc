...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> heights;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << heights << std::endl;
		std::cout.flush();

		__timer_start();

		MountainRanges __object;
		int __actual = __object.countPeaks(heights);
