...
vector <int>	wanted;
		...
		__in >> __expected >> A >> B >> wanted;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << A << ", " << B << ", " << wanted << std::endl;
		std::cout.flush();

		__timer_start();

		ComparerInator __object;
		int __actual = __object.makeProgram(A, B, wanted);
