...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> relations;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << relations << std::endl;
		std::cout.flush();

		__timer_start();

		CorporationSalary __object;
		long long __actual = __object.totalSalary(relations);
