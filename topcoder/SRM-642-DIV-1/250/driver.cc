...
int	s;
		...
		__in >> __expected >> time >> prob >> s;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << time << ", " << prob << ", " << s << std::endl;
		std::cout.flush();

		__timer_start();

		WaitingForBus __object;
		double __actual = __object.whenWillBusArrive(time, prob, s);
