...
vector <string>	problemTopics;
		...
		__in >> __expected >> attendance >> problemTopics;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << attendance << ", " << problemTopics << std::endl;
		std::cout.flush();

		__timer_start();

		TrainingCamp __object;
		vector <string> __actual = __object.determineSolvers(attendance, problemTopics);
