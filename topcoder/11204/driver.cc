...
vector <int>	overtoasted;
		...
		__in >> __expected >> undertoasted >> overtoasted;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << undertoasted << ", " << overtoasted << std::endl;
		std::cout.flush();

		__timer_start();

		ToastXToast __object;
		int __actual = __object.bake(undertoasted, overtoasted);
