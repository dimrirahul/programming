...
int	k;
		...
		__in >> __expected >> n >> m >> k;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << m << ", " << k << std::endl;
		std::cout.flush();

		__timer_start();

		JanuszInTheCasino __object;
		double __actual = __object.findProbability(n, m, k);
