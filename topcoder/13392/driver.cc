...
int	time;
		...
		__in >> __expected >> position >> count >> time;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << position << ", " << count << ", " << time << std::endl;
		std::cout.flush();

		__timer_start();

		CatsOnTheLineDiv2 __object;
		string __actual = __object.getAnswer(position, count, time);
