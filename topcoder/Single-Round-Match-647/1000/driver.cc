...
vector <int>	t;
		...
		__in >> __expected >> N >> K >> x >> t;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << K << ", " << x << ", " << t << std::endl;
		std::cout.flush();

		__timer_start();

		BuildingTowers __object;
		long long __actual = __object.maxHeight(N, K, x, t);
