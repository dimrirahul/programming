...
#include "TravellingSalesmanEasy.cc"
...
vector <int>	visit;
		...
		__in >> __expected >> M >> profit >> city >> visit;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << M << ", " << profit << ", " << city << ", " << visit << std::endl;
		std::cout.flush();

		__timer_start();

		TravellingSalesmanEasy __object;
		int __actual = __object.getMaxProfit(M, profit, city, visit);
