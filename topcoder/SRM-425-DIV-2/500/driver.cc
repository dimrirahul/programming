...
int	north;
		...
		__in >> __expected >> n >> east >> west >> south >> north;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << east << ", " << west << ", " << south << ", " << north << std::endl;
		std::cout.flush();

		__timer_start();

		CrazyBot __object;
		double __actual = __object.getProbability(n, east, west, south, north);
