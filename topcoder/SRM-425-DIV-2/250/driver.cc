...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> factors;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << factors << std::endl;
		std::cout.flush();

		__timer_start();

		InverseFactoring __object;
		int __actual = __object.getTheNumber(factors);
