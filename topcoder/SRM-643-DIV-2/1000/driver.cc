...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> parent;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << parent << std::endl;
		std::cout.flush();

		__timer_start();

		TheKingsTree __object;
		int __actual = __object.getNumber(parent);
