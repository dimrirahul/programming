...
#include "TheKingsFactorization.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		vector<long long>	__expected;
		long long	N;
vector<long long>	primes;

		__in >> __expected >> N >> primes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << primes << std::endl;
		std::cout.flush();

		__timer_start();

		TheKingsFactorization __object;
		vector<long long> __actual = __object.getVector(N, primes);
