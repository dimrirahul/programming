...
int	B;
		...
		__in >> __expected >> R >> G >> B;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << R << ", " << G << ", " << B << std::endl;
		std::cout.flush();

		__timer_start();

		MarbleDecoration __object;
		int __actual = __object.maxLength(R, G, B);
