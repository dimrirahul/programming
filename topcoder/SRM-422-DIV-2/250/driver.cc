...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> number;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << number << std::endl;
		std::cout.flush();

		__timer_start();

		MultiNumber __object;
		string __actual = __object.check(number);
