...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> goodSuffixes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << goodSuffixes << std::endl;
		std::cout.flush();

		__timer_start();

		TheLotteryBothDivs __object;
		double __actual = __object.find(goodSuffixes);
