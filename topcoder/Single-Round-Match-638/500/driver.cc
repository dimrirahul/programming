...
int	maxSizeSum;
		...
		__in >> __expected >> size >> maxSizeSum;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << size << ", " << maxSizeSum << std::endl;
		std::cout.flush();

		__timer_start();

		NarrowPassage2Easy __object;
		int __actual = __object.count(size, maxSizeSum);
