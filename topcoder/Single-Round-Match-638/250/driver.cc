...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> variableName;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << variableName << std::endl;
		std::cout.flush();

		__timer_start();

		NamingConvention __object;
		string __actual = __object.toCamelCase(variableName);
