...
#include "SolitaireSimulation.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> heaps;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << heaps << std::endl;
		std::cout.flush();

		__timer_start();

		SolitaireSimulation __object;
		int __actual = __object.periodLength(heaps);
