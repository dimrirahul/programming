...
vector <int>	holePos;
		...
		__in >> __expected >> initPos >> goalPos >> holePos;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << initPos << ", " << goalPos << ", " << holePos << std::endl;
		std::cout.flush();

		__timer_start();

		CubeRoll __object;
		int __actual = __object.getMinimumSteps(initPos, goalPos, holePos);
