...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> colonies;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << colonies << std::endl;
		std::cout.flush();

		__timer_start();

		BacteriesColony __object;
		vector <int> __actual = __object.performTheExperiment(colonies);
