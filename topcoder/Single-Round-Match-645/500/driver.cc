...
vector <int>	lengths;
		...
		__in >> __expected >> positions >> lengths;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << positions << ", " << lengths << std::endl;
		std::cout.flush();

		__timer_start();

		ConnectingCars __object;
		long long __actual = __object.minimizeCost(positions, lengths);
