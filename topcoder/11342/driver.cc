...
vector <int>	decisions;
		...
		__in >> __expected >> N >> decisions;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << decisions << std::endl;
		std::cout.flush();

		__timer_start();

		MafiaGame __object;
		double __actual = __object.probabilityToLose(N, decisions);
