...
#include "SlimeXSlimeRancher2.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> attributes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << attributes << std::endl;
		std::cout.flush();

		__timer_start();

		SlimeXSlimeRancher2 __object;
		int __actual = __object.train(attributes);
