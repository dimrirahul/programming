...
#include "Egalitarianism3Easy.cc"
...
vector <int>	len;
		...
		__in >> __expected >> n >> a >> b >> len;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << a << ", " << b << ", " << len << std::endl;
		std::cout.flush();

		__timer_start();

		Egalitarianism3Easy __object;
		int __actual = __object.maxCities(n, a, b, len);
