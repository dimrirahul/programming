...
#include "MagicalGirlLevelOneDivTwo.cc"
...
int	y;
		...
		__in >> __expected >> d >> x >> y;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << d << ", " << x << ", " << y << std::endl;
		std::cout.flush();

		__timer_start();

		MagicalGirlLevelOneDivTwo __object;
		double __actual = __object.theMinDistance(d, x, y);
