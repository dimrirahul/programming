...
int	layer_count;
		...
		__in >> __expected >> upper_limit >> layer_count;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << upper_limit << ", " << layer_count << std::endl;
		std::cout.flush();

		__timer_start();

		ToastXRaspberry __object;
		int __actual = __object.apply(upper_limit, layer_count);
