...
vector <int>	gridConnections;
		...
		__in >> __expected >> wires >> gridConnections;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << wires << ", " << gridConnections << std::endl;
		std::cout.flush();

		__timer_start();

		AddElectricalWires __object;
		int __actual = __object.maxNewWires(wires, gridConnections);
