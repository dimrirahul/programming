...
int	skillOfTeamB;
		...
		__in >> __expected >> skillOfTeamA >> skillOfTeamB;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << skillOfTeamA << ", " << skillOfTeamB << std::endl;
		std::cout.flush();

		__timer_start();

		PrimeSoccer __object;
		double __actual = __object.getProbability(skillOfTeamA, skillOfTeamB);
