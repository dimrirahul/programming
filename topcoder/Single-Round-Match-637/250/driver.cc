...
vector <int>	sothe;
		...
		__in >> __expected >> snuke >> sothe;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << snuke << ", " << sothe << std::endl;
		std::cout.flush();

		__timer_start();

		GreaterGameDiv2 __object;
		int __actual = __object.calc(snuke, sothe);
