...
#include "IngredientProportions.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		vector <string>	proportions;

		__in >> __expected >> proportions;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << proportions << std::endl;
		std::cout.flush();

		__timer_start();

		IngredientProportions __object;
		vector <int> __actual = __object.getMasses(proportions);
