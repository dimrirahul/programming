...
#include "SubrectanglesOfTable.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> table;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << table << std::endl;
		std::cout.flush();

		__timer_start();

		SubrectanglesOfTable __object;
		vector<long long> __actual = __object.getQuantity(table);
