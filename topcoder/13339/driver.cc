...
vector <int>	Number2;
		...
		__in >> __expected >> Type1 >> Number1 >> Type2 >> Number2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << Type1 << ", " << Number1 << ", " << Type2 << ", " << Number2 << std::endl;
		std::cout.flush();

		__timer_start();

		CandyCollection __object;
		int __actual = __object.solve(Type1, Number1, Type2, Number2);
