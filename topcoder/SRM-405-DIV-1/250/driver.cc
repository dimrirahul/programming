...
string	currentDir;
		...
		__in >> __expected >> path >> currentDir;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << path << ", " << currentDir << std::endl;
		std::cout.flush();

		__timer_start();

		RelativePath __object;
		string __actual = __object.makeRelative(path, currentDir);
