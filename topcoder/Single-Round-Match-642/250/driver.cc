...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> expression;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << expression << std::endl;
		std::cout.flush();

		__timer_start();

		ForgetfulAddition __object;
		int __actual = __object.minNumber(expression);
