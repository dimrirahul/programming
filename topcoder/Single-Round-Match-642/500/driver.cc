...
#include "LightSwitchingPuzzle.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> state;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << state << std::endl;
		std::cout.flush();

		__timer_start();

		LightSwitchingPuzzle __object;
		int __actual = __object.minFlips(state);
