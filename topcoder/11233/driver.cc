...
int	repetitions;
		...
		__in >> __expected >> ballSequence >> repetitions;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << ballSequence << ", " << repetitions << std::endl;
		std::cout.flush();

		__timer_start();

		MathContest __object;
		int __actual = __object.countBlack(ballSequence, repetitions);
