...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> fieldOrder;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << fieldOrder << std::endl;
		std::cout.flush();

		__timer_start();

		FIELDDiagrams __object;
		long long __actual = __object.countDiagrams(fieldOrder);
