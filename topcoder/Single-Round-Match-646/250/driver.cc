...
#include "TheConsecutiveIntegersDivTwo.cc"
...
int	k;
		...
		__in >> __expected >> numbers >> k;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << numbers << ", " << k << std::endl;
		std::cout.flush();

		__timer_start();

		TheConsecutiveIntegersDivTwo __object;
		int __actual = __object.find(numbers, k);
