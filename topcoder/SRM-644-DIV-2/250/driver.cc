...
int	K;
		...
		__in >> __expected >> osize >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << osize << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		OkonomiyakiShop __object;
		int __actual = __object.count(osize, K);
