...
int	y2;
		...
		__in >> __expected >> width >> height >> xfold >> cnt >> x1 >> y1 >> x2 >> y2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << width << ", " << height << ", " << xfold << ", " << cnt << ", " << x1 << ", " << y1 << ", " << x2 << ", " << y2 << std::endl;
		std::cout.flush();

		__timer_start();

		PaperAndPaintEasy __object;
		long long __actual = __object.computeArea(width, height, xfold, cnt, x1, y1, x2, y2);
