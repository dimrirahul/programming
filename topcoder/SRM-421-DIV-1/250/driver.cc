...
vector <int>	m;
		...
		std::cout << "# input for test " << __testnum << ": " << x << ", " << m << std::endl;
		std::cout.flush();

		__timer_start();

		EquilibriumPoints __object;
		vector <double> __actual = __object.getPoints(x, m);
