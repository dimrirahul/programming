...
int	maxCuts;
		...
		__in >> __expected >> weights >> maxCuts;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << weights << ", " << maxCuts << std::endl;
		std::cout.flush();

		__timer_start();

		CakesEqualization __object;
		double __actual = __object.fairDivision(weights, maxCuts);
